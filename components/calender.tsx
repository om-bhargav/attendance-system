    ...
    <div className="shadow-lg flex flex-col gap-1 w-full lg:w-auto">
      ...
      <div className="p-3 grid grid-cols-7 gap-3">
        ...
        {[...Array(days_of_month)].map((item, index) => (
          ...
          className={`${holidays!==undefined && holidays.includes((month_index+1)+"-"+(index+1)+"-"+current.getFullYear())? "bg-green-500! cursor-pointer" :[0,6].includes((first_day_of_month+index)%7)?"bg-gray-500! cursor-not-allowed pointer-events-none":"cursor-pointer bg-red-800!"} text-white p-2 text-center rounded flex justify-center`}
          >
            {index + 1}
          </div>
        ))}
